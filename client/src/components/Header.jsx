    ...
    <div className="sticky top-0 left-0 z-50 bg-white font-['Cairo']">
      ...
      {isOpened && <div className="absolute  top-0 right-0 bottom-0 overflow-hidden bg-white transition-all w-full h-screen">
        <div className="flex flex-col text-gray-600">
          ...
          onClick={() => { setIsOpened(false);setPage("HOME") }} to="/" aria-current="page">الرئيسية</Link>
          <Link className={`py-2 pl-6 border-t ${page === "COLLECTIONS" && "bg-black text-white"}`} 
          onClick={() => { setIsOpened(false);setPage("COLLECTIONS") }} to="/collections">المجموعات</Link>
          <Link className={`py-2 pl-6 border-t ${page === "ABOUT" && "bg-black text-white"}`} 
          onClick={() => { setIsOpened(false);setPage("ABOUT") }} to="/about">عنا</Link>
          <Link className={`py-2 pl-6 border-t ${page === "CONTACT" && "bg-black text-white"}`}
           onClick={() => { setIsOpened(false);setPage("CONTACT") }} to="/contact">اتصال</Link>
